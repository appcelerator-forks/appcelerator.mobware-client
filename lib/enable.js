...
module.exports = function enable(username, password, opts, callback) {
	...
	// closures for use with async
	var authenticateClosure = function(cb) {
		return authenticateRequest(username, password, cb);
	},
	enableClosure = function(cookie, cb) {
		return enableRequest(opts.appId || '', opts.tiapp, cookie, cb);
	};

	// auth then enable app
	async.waterfall([ authenticateClosure, enableClosure ], function(err, results) {
...
function doRequest(api, opts, callback) {
	...
	var postData = querystring.stringify(opts.postObject || {}),
		httpsOpts = {
			host: constants.MW_HOST,
			port: constants.MW_PORT,
			method: 'POST',
			path: api,
			headers: {
...
function authenticateRequest(username, password, callback) {
	return doRequest(
		constants.AUTH_API,
		{
			postObject: {
...
function enableRequest(appId, tiappFile, cookie, callback) {
	return doRequest(
